...
class FCoTOrchestrator:
    ...
    def __init__(self, registry: AgentRegistry, evaluator: DualObjectiveEvaluator):
        ...
        # Stateful Orchestration Tracker (adapted from orchestration.md)
        self.orchestration_state = {
            "vibration_completed": False,
    ...
    def execute_reasoning_loop(self, asset_id: str, goal_intent: str) -> List[Dict[str, Any]]:
        ...
        # =====================================================================
        # PRE-EXECUTION: MISSION SCOPE DECLARATION (adapted from orchestration.md)
        # =====================================================================
        logger.info("\n" + "="*80 + "\n[FCoT PRE-EXECUTION] MISSION SCOPE & BOUNDARY CONDITIONS DECLARATION\n" + "="*80)
        logger.info("1. FIXED BOUNDARIES:")
        logger.info("   └─ Plant Nominal Megawatts = 500.0 MW (Grid constraint)")
        logger.info("   └─ Auxiliary bearing cooling loops = 100% healthy")
        logger.info("   └─ Warehouse logistics lead times & Crew shifts = locked")
        logger.info("2. DECOMPOSABLE VARIABLES:")
        logger.info("   └─ Pump 2A Operating Load % (Micro)")
        logger.info("   └─ Standby Pump 2B Redundancy activation state (Meso)")
        logger.info("   └─ Control Room Hot-swap sequence trigger (Macro)")
        logger.info("3. OBJECTIVE WEIGHTS:")
        logger.info("   └─ f_max (Maximize OEE)      : Weight = 0.5")
        logger.info("   └─ f_min (Minimize Safe Risk): Weight = 0.5")
        logger.info("="*80 + "\n")
        ...
        # Check prerequisites statefully
        if self.orchestration_state["vibration_completed"]:
            logger.warning("Vibration analysis already completed. Skipping redundancy block.")
        else:
            thought_1 = (
            ...
            self.orchestration_state["vibration_completed"] = True
            logger.info(f"State Tracker Updated: {self.orchestration_state}")

            # Micro-Level State: Immediate Shutdown.
            state_1 = PlantState(
                asset_id=asset_id,
                operating_load_pct=0.0,          
                vibration_rms=0.0,
                bearing_temp=25.0,
                redundancy_active=False,         
                production_mw=250.0,             
                maintenance_delay_hours=0.0,     
        ...
        # Verify stateful routing prerequisites
        if not self.orchestration_state["vibration_completed"]:
            logger.error("State violation: Cannot execute Meso-level logic before Micro-level diagnostics are complete.")
            raise ValueError("State validation failed: Micro-level diagnostics missing.")
            
        if self.orchestration_state["supply_chain_completed"]:
            logger.warning("Supply chain analysis already completed. Skipping block.")
        else:
            thought_2 = (
            ...
            # Update orchestration state
            self.orchestration_state["supply_chain_completed"] = True
            logger.info(f"State Tracker Updated: {self.orchestration_state}")
        ...
        # Verify stateful routing prerequisites
        if not self.orchestration_state["supply_chain_completed"]:
            logger.error("State violation: Cannot execute Macro-level logic before Meso-level logistics are complete.")
            raise ValueError("State validation failed: Meso-level logistics missing.")
            
        if self.orchestration_state["dcs_completed"]:
            logger.warning("DCS analysis already completed. Skipping block.")
        else:
            thought_3 = (
            ...
            # Update orchestration state
            self.orchestration_state["dcs_completed"] = True
            logger.info(f"State Tracker Final Update: {self.orchestration_state}")
